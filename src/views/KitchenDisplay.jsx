...
import { markOrderPreparing, markOrderReady, finishOrder } from "../services/api.service";

// Convierte "Pending" -> 0, "Preparing" -> 1, "Ready" -> 2, etc.
// Necesario porque .NET puede mandar el enum como string o como número
...
const KitchenDisplay = () => {
  ...
  // Normalizar status antes de filtrar
  ...
  const pending   = normalized.filter((o) => o.status === 0).sort(byTime);
  const preparing = normalized.filter((o) => o.status === 1).sort(byTime);
  const ready     = normalized.filter((o) => o.status === 2).sort(byTime);
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white p-6">
      ...
      <div className="grid grid-cols-3 gap-6">
        <Column title="Pending"   orders={pending}   now={now} isConnected={isConnected} getOrderId={getOrderId} />
        <Column title="Preparing" orders={preparing} now={now} isConnected={isConnected} getOrderId={getOrderId} />
        <Column title="Ready"     orders={ready}     now={now} isConnected={isConnected} getOrderId={getOrderId} />
      </div>
    </div>
  );
};
